...
```bash
# 基本用法
scripts/writer humanize article.md

# 指定处理强度
scripts/writer humanize article.md --intensity gentle
scripts/writer humanize article.md --intensity aggressive

# 显示修改对比和质量评分
scripts/writer humanize article.md --show-changes

# 输出到文件
scripts/writer humanize article.md -o output.md
```
...
```bash
# 写作 + 去痕
scripts/writer write --style dan-koe --humanize

# 指定去痕强度
scripts/writer write --style dan-koe --humanize --humanize-intensity aggressive
```
...
```
scripts/用户: writer write --style dan-koe --humanize
